...
elif pnpm --silent exec -- rolldown --version >/dev/null 2>&1; then
  pnpm --silent exec -- rolldown -c "$A2UI_APP_DIR/rolldown.config.mjs"
else
  npx --yes rolldown@latest -c "$A2UI_APP_DIR/rolldown.config.mjs"
fi
